...
const navItems = [
  ...
  { icon: MessageSquare, label: "Sessions", href: "/dashboard/sessions", active: false },
  ...
  return (
    <div className="min-h-screen bg-transparent flex">
      ...
      <button
        ...
        className="fixed top-4 left-4 z-50 lg:hidden bg-[#0d1110] border border-[#ffffff0a] rounded-lg p-2 text-gray-400 hover:text-white transition-colors cursor-pointer"
      >
      ...
      {/* Sidebar */}
      <aside
        className={`fixed lg:sticky top-0 left-0 h-screen w-[260px] bg-[#090b0a] border-r border-[#ffffff0a] flex flex-col z-40 transition-transform duration-300 lg:translate-x-0 ${
      ...
      >
        ...
        <div className="px-5 py-4 border-b border-[#ffffff0a]">
          <div className="flex items-center gap-3">
            <div className="w-8 h-8 rounded-full bg-white/10 border border-[#ffffff14] flex items-center justify-center">
              <span className="text-xs font-semibold text-gray-200">Y</span>
            </div>
            <div className="min-w-0">
              <div className="text-sm font-medium text-white truncate">
                Friendly Dev
              </div>
              <div className="text-xs text-gray-400">Trialing</div>
            </div>
          </div>
        </div>
        ...
        <nav className="flex-1 px-3 py-4 space-y-1">
          {navItems.map((item) => (
            ...
            >
              {item.active && (
                <div className="absolute left-0 w-[3px] h-5 bg-gray-300 rounded-r-full" />
              )}
          ...
          {externalLinks.map((item) => (
            <a
              ...
              className="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm text-gray-400 hover:text-white hover:bg-white/5 transition-all duration-150 group"
            >
              <item.icon className="w-4 h-4 shrink-0" />
              {item.label}
        ...
        <div className="px-3 pb-4">
          <button
            ...
            className="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm text-gray-400 hover:text-red-400 hover:bg-red-500/5 transition-all duration-150 w-full cursor-pointer"
          >
      ...
      <main className="flex-1 min-w-0">
        ...
        <div className="h-16 border-b border-[#ffffff0a] flex items-center px-6 lg:px-8">
          ...
          <div>
            <h1 className="text-xl font-semibold text-[#fff1e0] tracking-tight">Welcome back, Builder 👋</h1>
        ...
        <div className="px-6 lg:px-8 py-8 max-w-5xl">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
            ...
            <motion.div
              ...
              className="bg-[#0d1110] border-l-4 border-l-gray-300 border border-[#ffffff0a] rounded-2xl p-6"
            >
            ...
            >
              ...
              <p className="text-sm text-gray-400 mb-4">7 days remaining</p>

              <Link
                href="/billing"
                className="inline-flex items-center gap-1.5 text-sm text-gray-200 hover:text-gray-400 transition-colors font-medium group"
              >
                Upgrade to Pro
            ...
            <div className="bg-[#0d1110] border border-[#ffffff0a] rounded-2xl p-5">
              <div className="flex items-center justify-between mb-3">
                ...
                >
                  {copied ? (
                    <>
                      <Check className="w-3.5 h-3.5 text-krud-green" />
                      <span className="text-krud-green">Copied</span>
                    </>
                  ) : (
                    <>
